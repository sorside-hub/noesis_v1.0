...
class WikilinkTextWidget extends WidgetType {
  ...
  toDOM() {
    ...
    const textClass = this.exists
      ? 'text-link-primary font-medium hover:opacity-80 cursor-pointer inline-block'
      : 'text-link-primary/60 hover:text-link-primary font-medium cursor-pointer inline-block';
...
export function getWikilinkDecorations(
  ...
  while ((match = regex.exec(docText)) !== null) {
    ...
    const textClass = exists
      ? 'text-link-primary font-medium hover:opacity-80 cursor-pointer'
      : 'text-link-primary/60 font-medium cursor-pointer';
    ...
    if (isSourceMode) {
      ...
      if (pipeRelativeIndex !== -1) {
        ...
        decorations.push(
          Decoration.mark({
            class: 'text-link-primary font-medium',

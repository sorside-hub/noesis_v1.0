...
const renderer = {
  ...
  link({ href, title, tokens }: { href: string; title?: string | null; tokens: any[] }) {
    ...
    return `<a href="${href}"${titleAttr} target="_blank" rel="noopener noreferrer" class="text-link-primary underline hover:opacity-80 transition-opacity">${text}</a>`;
  }
};
  ...
  try {
    ...
    processedHtml = processedHtml.replace(/<span data-internal-wikilink-raw="(.*?)"><\/span>/g, (match, encodedTarget) => {
      ...
      const styleClass = exists
        ? 'wikilink-item resolved text-link-primary font-medium no-underline hover:opacity-85 cursor-pointer transition-opacity'
        : 'wikilink-item ghost text-link-primary/60 hover:text-link-primary font-medium no-underline cursor-pointer transition-colors';

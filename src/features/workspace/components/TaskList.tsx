      ...
      {filteredTasks.map((task) => (
        ...
        >
          ...
          <button
            ...
            className="mt-0.5 text-icon-secondary hover:text-accent-primary transition-colors cursor-pointer shrink-0"
            title={task.isCompleted ? 'Mark uncompleted' : 'Mark completed'}
          >
            {task.isCompleted ? (
              <CheckCircle2 size={16} className="text-status-success fill-status-success/20" />
            ) : (
              <Circle size={16} className="text-icon-secondary hover:text-accent-primary" />
            )}
          </button>
          ...
          >
            ...
            <div className="flex items-center gap-2 mt-1 text-[10px] text-text-muted opacity-0 group-hover:opacity-100 transition-opacity">
              <span className="flex items-center gap-0.5">
                <CornerDownRight size={10} className="text-icon-secondary" />

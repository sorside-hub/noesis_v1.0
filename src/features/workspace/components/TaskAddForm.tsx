...
export const TaskAddForm: React.FC<TaskAddFormProps> = ({
    ...
    return (
      <form onSubmit={onAddNewTask} className="space-y-2 pt-1">
        <input
          ...
          autoFocus
          className="w-full px-3 py-1.5 bg-bg-primary border border-accent-primary rounded-lg text-xs text-text-primary placeholder:text-text-muted focus:outline-none"
        />
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-center gap-1.5 py-2 border border-dashed border-border-default hover:border-accent-primary/60 rounded-xl text-xs font-medium text-text-secondary hover:text-accent-primary hover:bg-accent-soft transition-all cursor-pointer"
    >
      <Plus size={13} />

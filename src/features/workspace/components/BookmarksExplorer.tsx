      ...
      <div key={b.id} className="relative group select-none">
        ...
        >
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {node.type === 'folder' ? (
              <Folder size={13} className={twMerge('shrink-0', isSelected ? 'text-text-inverse' : 'text-icon-accent')} />
            ) : (
              <FileText
                size={13}
                className={twMerge(
            ...
            {isEditing ? (
              ...
              <span className={twMerge("truncate flex-1 text-[11.5px]", isSelected ? "text-text-inverse font-medium" : "text-text-secondary group-hover:text-text-primary")}>
                {b.title || node.name}
              </span>
            )}
          </div>
        ...
        {isMenuOpen && (
          <div
            data-bookmark-menu="true"
            className="absolute right-2 top-full mt-1 z-30 bg-bg-quaternary border border-border-default rounded-xl shadow-lg py-1.5 min-w-[160px] text-xs animate-in fade-in zoom-in-95 duration-100 font-normal"
            onClick={(e) => e.stopPropagation()}
          >
            {onOpenBookmarkModal && (
              <button
                ...
                className="group w-full flex items-center gap-2 px-3 py-1.5 text-text-secondary hover:text-text-primary hover:bg-bg-hover transition-colors text-left cursor-pointer"
              >
                <Bookmark size={12} className="text-icon-accent group-hover:text-text-primary shrink-0 transition-colors" />
            ...
            <button
              ...
              className="group w-full flex items-center gap-2 px-3 py-1.5 text-text-secondary hover:text-text-primary hover:bg-bg-hover transition-colors text-left cursor-pointer"
            >
              <Edit2 size={12} className="text-icon-accent group-hover:text-text-primary shrink-0 transition-colors" />
              ...
              <>
                ...
                {b.groupId && (
                  <button
                    ...
                    className="group w-full flex items-center gap-2 px-3 py-1.5 text-text-secondary hover:text-text-primary hover:bg-bg-hover transition-colors text-left cursor-pointer text-[11px]"
                  >
                    <Folder size={12} className="text-icon-accent group-hover:text-text-primary shrink-0 transition-colors" />
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-1.5 text-status-error hover:bg-status-error-bg/30 transition-colors text-left cursor-pointer font-medium"
            >
              <Trash2 size={12} className="text-status-error shrink-0" />
      ...
      <div key={group.id} className="space-y-0.5">
        ...
        >
          <div className="flex items-center gap-1.5 min-w-0 flex-1">
            <span className="p-0.5 -ml-0.5 text-text-secondary transition-transform duration-150 shrink-0">
              {isExpanded ? (
            ...
            {isEditing ? (
              ...
              >
                <input
                  ...
                  className="bg-bg-surface border border-border-default rounded px-1.5 py-0.5 text-xs text-text-primary w-full outline-hidden font-normal"
                />
              </form>
            ) : (
          ...
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <span className="px-1.5 py-0.2 text-[10px] font-semibold rounded-full bg-bg-quaternary text-text-secondary group-hover:text-text-primary border border-border-default/50 tabular-nums">
              {groupItems.length}
            </span>
            ...
            {!isEditing && (
              <button
                ...
                className="p-1 rounded text-icon-accent hover:text-text-primary hover:bg-bg-hover transition-all cursor-pointer"
              >
                <MoreVertical size={12} />
              </button>
            )}
          ...
          {isMenuOpen && (
            <div
              data-bookmark-menu="true"
              className="absolute right-2 top-full mt-1 z-30 bg-bg-quaternary border border-border-default rounded-xl shadow-lg py-1.5 min-w-[150px] text-xs animate-in fade-in zoom-in-95 duration-100 font-normal"
              onClick={(e) => e.stopPropagation()}
            >
              <button
                type="button"
        ...
        {/* Group Items */}
        {isExpanded && (
          <div className="relative border-l border-border-default/40 ml-3.5 my-0.5 space-y-0.5 pl-2">
            {groupItems.length === 0 ? (
              <div className="py-1 px-2 text-[11px] text-text-secondary/70 italic">
    ...
    >
      ...
      <div className="px-2 py-1 flex items-center justify-between text-[11px] font-medium text-text-secondary border-b border-border-default/40 pb-1.5 mb-1">
        <span className="flex items-center gap-1.5 text-text-secondary">
          <Star size={12} className="text-icon-accent" />
          <span>
            {query
      ...
      {isCreatingGroup && (
        <form
          ...
          className="mx-2 mb-2 p-2.5 bg-bg-quaternary border border-border-default rounded-xl space-y-2 animate-in fade-in duration-100 shadow-lg"
        >
          <div className="flex items-center gap-1.5 text-xs text-text-primary font-medium">
            <FolderPlus size={13} className="text-icon-accent" />
          ...
          <input
            ...
            className="w-full bg-bg-surface border border-border-default rounded-lg px-2.5 py-1.5 text-xs text-text-primary placeholder:text-text-secondary/60 outline-hidden focus:ring-1 focus:ring-accent-primary focus:border-accent-primary"
          />
          <div className="flex items-center justify-end gap-1.5 pt-0.5">
            <button
              ...
              className="px-2.5 py-1 rounded-md text-[11px] text-text-secondary hover:text-text-primary hover:bg-bg-hover cursor-pointer"
            >
              Batal
            </button>
            <button
              ...
              className="px-3 py-1 rounded-md text-[11px] font-medium bg-accent-primary text-text-inverse hover:opacity-90 disabled:opacity-50 cursor-pointer"
            >
              Buat Grup
            </button>
          </div>
      ...
      {query && totalMatchingCount === 0 && (
        <div className="flex flex-col items-center justify-center p-6 text-center text-text-secondary select-none mt-4">
          <div className="w-8 h-8 rounded-full bg-bg-quaternary border border-border-default flex items-center justify-center text-icon-accent mb-2">
            <Search size={16} />
          </div>
          <p className="text-xs font-semibold text-text-primary mb-1">Penanda Tidak Ditemukan</p>
          <p className="text-[11px] leading-relaxed text-text-secondary/80 max-w-[200px]">
      ...
      {!query && bookmarks.length === 0 && groups.length === 0 && (
        <div className="flex flex-col items-center justify-center p-6 text-center text-text-secondary select-none mt-8">
          <div className="w-10 h-10 rounded-full bg-bg-quaternary border border-border-default flex items-center justify-center text-icon-accent mb-3">
            <Bookmark size={20} />
          </div>

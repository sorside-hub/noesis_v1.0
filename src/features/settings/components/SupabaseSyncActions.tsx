...
export const SupabaseSyncActions: React.FC<SupabaseSyncActionsProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {syncPhase !== 'idle' && (
        <div className="p-4 rounded-xl bg-bg-primary space-y-2 animate-in fade-in duration-200">
      ...
      {syncError && (
        <div className="flex items-start gap-2.5 p-3.5 text-xs text-status-error bg-status-error-bg rounded-xl">
      ...
      {syncMessage && !syncError && syncPhase === 'idle' && (
        <div className="p-3.5 rounded-xl bg-emerald-500/10 text-emerald-400 text-xs flex items-center gap-2.5 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
        <button
          ...
          className="flex items-center justify-center gap-2 p-3 text-xs font-semibold text-text-primary bg-bg-primary hover:bg-bg-hover rounded-xl transition-all shadow-2xs cursor-pointer disabled:opacity-50"
        >
          {syncPhase === 'pull' ? (
            <Loader2 className="w-4 h-4 animate-spin text-accent-primary" />
          ) : (

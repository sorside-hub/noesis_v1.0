...
export const InboxTriageColumn: React.FC<InboxTriageColumnProps> = ({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col h-full rounded-xl relative overflow-hidden transition-all duration-200 flex-shrink-0 w-72 ${
        isOver
          ? `bg-bg-secondary ${details.dropActive} shadow-xl scale-[1.01]`
    ...
    >
      ...
      {/* Column Header with bg-bg-secondary and preserved border-b */}
      ...
      <div className="flex-1 p-2.5 overflow-y-auto custom-scrollbar flex flex-col gap-2.5 min-h-[150px]">
        ...
        >
          {notes.length > 0 ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center rounded-xl bg-bg-primary/50 text-text-muted">

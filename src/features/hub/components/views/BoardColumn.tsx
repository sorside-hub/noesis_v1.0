...
export const getColumnTheme = (colKey: string): ColumnTheme => {
  ...
  if (normalized === 'idea' || normalized === 'ideas' || normalized === 'backlog') {
    return {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-amber-500/40 bg-amber-500/10',
    };
  }
  if (normalized === 'draft' || normalized === 'todo' || normalized === 'to do') {
    return {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-sky-500/40 bg-sky-500/10',
    };
  }
  if (normalized === 'in progress' || normalized === 'progress' || normalized === 'doing' || normalized === 'active') {
    return {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-purple-500/40 bg-purple-500/10',
    };
  }
  if (normalized === 'completed' || normalized === 'done' || normalized === 'finished') {
    return {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-emerald-500/40 bg-emerald-500/10',
    };
  }
  if (normalized === 'archived' || normalized === 'archive' || normalized === 'closed') {
    return {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-slate-400/40 bg-slate-500/10',
    };
  }
  ...
  const palettes: ColumnTheme[] = [
    {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-rose-500/40 bg-rose-500/10',
    },
    {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-teal-500/40 bg-teal-500/10',
    },
    {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-orange-500/40 bg-orange-500/10',
    },
    {
      ...
      borderHover: '',
      dropRing: 'ring-2 ring-cyan-500/40 bg-cyan-500/10',
    }
  ];
...
export const BoardColumn: React.FC<BoardColumnProps> = ({
  ...
  return (
    <div 
      className={`flex flex-col h-full rounded-xl relative overflow-hidden transition-all duration-200 ${
        isSingleColumn ? 'w-full flex-1' : 'flex-shrink-0 w-72'
      } ${
        isOver 
          ? `bg-bg-secondary ${theme.dropRing} shadow-xl scale-[1.01]` 
    ...
    >
      ...
      {/* Column Header */}
      <div className="flex items-center justify-between px-3.5 py-3 border-b border-border-default/40 bg-bg-secondary">
      ...
      {/* Column Content */}
      <div ref={setNodeRef} className="flex-1 overflow-y-auto p-2.5 space-y-2.5 custom-scrollbar flex flex-col relative min-h-[150px]">
        ...
        {notes.length === 0 && (
          <div className="w-full p-4 flex items-center justify-center rounded-lg bg-bg-primary/50 text-text-muted text-xs italic opacity-60 mt-2">
            Kosong
          </div>
        )}
      </div>
